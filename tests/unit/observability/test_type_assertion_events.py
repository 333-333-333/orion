...
def test_type_assertion_events_started_completed_and_no_raw_text_memory_sink():
    ...
    sut.process("John is a person. Apple is an organization.")

    events = [e.to_dict() for e in sink.events if e.phase == "type_assertion"]
...
def test_type_assertion_failed_event_and_no_raw_text_jsonl(tmp_path):
    import json
    from observability import JsonlFileLogSink
    from orion import ORION

    class _ExplodeTypeAssertionORION(ORION):
        def _run_type_assertion(self, payload):
            raise RuntimeError("boom-type-assertion")

    out_file = tmp_path / "type-assertion-events.jsonl"
    sut = _ExplodeTypeAssertionORION(config={"logging": {"sink": JsonlFileLogSink(out_file)}, "spacy_model": "en_core_web_lg"})

    with pytest.raises(RuntimeError):
        sut.process("John is a person.")

    payloads = [json.loads(line) for line in out_file.read_text(encoding="utf-8").splitlines()]
    events = [p for p in payloads if p["phase"] == "type_assertion"]
