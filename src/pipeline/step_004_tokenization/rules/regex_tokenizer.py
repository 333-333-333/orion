"""Define regex tokenization and deterministic token identifiers."""
...
TOKEN_PATTERN = re.compile(r"\w+|(?<=\w)['’][sS]\b|[^\w\s]", re.UNICODE)


def build_token_id(source_text_id: str, index: int, sentence_id: str, start_offset: int, end_offset: int, text: str) -> str:
    """Build a deterministic token identifier from source and span data."""
