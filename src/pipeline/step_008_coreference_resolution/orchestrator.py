...
_BR_COREF_RELATIVE_PRONOUNS = {"that", "which", "who"}
...
def _noun_chunks_for_sentence(noun_chunks: list[dict[str, Any]], sentence_id: str) -> list[dict[str, Any]]:
    chunks = [c for c in noun_chunks if c.get("sentence_id") == sentence_id]
    return sorted(chunks, key=lambda c: (c.get("start_offset", -1), c.get("end_offset", -1), _normalize_text(c.get("text", ""))))
...
def _score_candidate(mention: dict[str, Any], chunk: dict[str, Any]) -> tuple[float, dict[str, float]]:
    ...
    # Confidence determinista: suma ponderada fija (distance/position/bonus), sin azar ni estado externo.
...
def _resolve_mention(mention: dict[str, Any], sentence_chunks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    candidates.sort(
        key=lambda item: (
            ...
            _normalize_text(item[2].get("text", "")),
        ),
        reverse=True,
    )
...
def resolve_coreferences_from_payload(input_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for token in tokens:
        if _normalize_text(token.get("text", "")) not in _BR_COREF_RELATIVE_PRONOUNS:
            continue
        sentence_id = token.get("sentence_id", "")
        sentence_chunks = _noun_chunks_for_sentence(noun_chunks, sentence_id)
        coreferences.append(_resolve_mention(token, sentence_chunks))
